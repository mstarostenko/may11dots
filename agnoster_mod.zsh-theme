...
prompt_mod_dir() {
  ...
  [[ "$PWD" -regex-match "^$HOME([/]?)" ]] && in_home_dir=1 || in_home_dir=0

  if [[ $in_home_dir -eq 1 ]] {
    dir_path_color="$LIGHTMAGENTA"
    dir_icon_color="$MAGENTA"
  }

  if [[ $in_home_dir -eq 0 ]] {
    dir_path_color="$BRIGHTYELLOW"
    dir_icon_color="$BRIGHTYELLOW"
  }
...
prompt_username() {
    echo -n "%F{125}%f%B%F{132}%n%f%b"
}

prompt_exit_status() {
  [[ $RETVAL -ne 0 ]] && echo -n "%{%F{red}%}╘✘$RETVAL%f"
